...
public class Payment {
	...
	public Map<String, Integer[]> product_list(int[] list) {
		...
		try {
			...
			PreparedStatement pstmt = conn.prepareStatement("SELECT * FROM shoes WHERE shoes_serial_number = ?");
			ResultSet rs = null;
	...
	public int pay(Map<String, Integer[]> productlist) {
		...
		try {
			...
			PreparedStatement pstmt = conn.prepareStatement(
					"UPDATE shoes SET st_info = st_info-?, st_sales = st_sales+? WHERE shoes_serial_number LIKE ?");
			ResultSet rs = null;
...
//			pstmt = conn.prepareStatement("INSERT INTO product_record VALUES (sales_number.nextval,"  // 시퀀스번호
			...
			pstmt = conn.prepareStatement(
					"SELECT sales_number FROM (SELECT * FROM product_record ORDER BY sales_number DESC) WHERE rownum = 1");
			rs = pstmt.executeQuery();
			rs.next();
	...
	public int MemberPay(Map<String, Integer[]> productlist, String cardnumber) {
		...
		try {
			...
			PreparedStatement pstmt = conn.prepareStatement(
					"UPDATE shoes SET st_info = st_info-?, st_sales = st_sales+? WHERE shoes_serial_number = ?");
			ResultSet rs = null;
			...
			pstmt = conn.prepareStatement("SELECT * " + "FROM (SELECT * FROM ABC" + cardnumber + " ORDER BY sh_id DESC)"
					+ "WHERE ROWNUM = 1");
			rs = pstmt.executeQuery();
			if (rs.next()) {
			...
			pstmt = conn.prepareStatement("INSERT INTO ABC" + cardnumber + " VALUES (" + sales_number + ", " // 판매번호
					+ "?, " // 신발이름
					+ "?, " // 신발수량
					+ "?, " // 신발사이즈
					+ "?, " // 신발가격
	...
	public int refund(String sal_num, String serial, String info) {
		...
		try {
			...
			pstmt = conn.prepareStatement("SELECT * FROM product_record " + "WHERE sales_number = ? AND "
					+ "shoes_serial_number = ? AND " + "sh_info >= ?");
			...
			if (rs.next()) {
				...
				if (rs.getInt("sh_info") - Integer.parseInt(info) <= 0) {
					pstmt = conn.prepareStatement(
							"DELETE FROM product_record " + "WHERE sales_number = ? AND " + "shoes_serial_number = ? ");
					pstmt.setInt(1, rs.getInt("sales_number"));
					pstmt.setInt(2, rs.getInt("shoes_serial_number"));
					ResultSet rs2 = pstmt.executeQuery();
					rs2.close();
				} else {
					pstmt = conn.prepareStatement("UPDATE product_record "
							+ "set sh_price = sh_price - (sh_price / sh_info * ?), sh_info = sh_info-? "
							+ "WHERE sales_number = ? AND " + "shoes_serial_number = ?");
	...
	public int Member_refund(String member, String sal_num, String serial, String info) {
		...
		try {
			...
			pstmt = conn.prepareStatement(
					"SELECT * FROM ABC" + member + " WHERE sales_number = ? AND shoes_serial_number = ?");
			int mpoint;
			...
			if (rs5.next()) {
				mpoint = (int) Math
						.floor((rs5.getInt("sh_price") / rs5.getInt("sh_count")) * 0.02 * Integer.parseInt(info));
				System.out.println(mpoint);
				if (rs5.getInt("sh_count") - Integer.parseInt(info) <= 0) {
					pstmt = conn.prepareStatement(
							"DELETE FROM ABC" + member + " WHERE sales_number = ? AND " + "shoes_serial_number = ? ");
					pstmt.setInt(1, rs5.getInt("sales_number"));
					pstmt.setInt(2, rs5.getInt("shoes_serial_number"));
					ResultSet rs2 = pstmt.executeQuery();
					rs2.close();
				} else {
					pstmt = conn.prepareStatement("UPDATE ABC" + member
				...
				pstmt = conn.prepareStatement("UPDATE ABC" + member + " set current_point = current_point - " + mpoint

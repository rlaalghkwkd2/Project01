...
public class Record {
	...
	public void view(JTextArea ta) {
		...
		try {
			...
			pstmt = conn.prepareStatement("SELECT * FROM product_record " + "WHERE TO_CHAR(sales_time,'YYYYMMDD') = "
					+ today + " ORDER BY sales_number DESC");
			rs = pstmt.executeQuery();
			while (rs.next()) {
				String tmp = "";
				if (rs.getString("sh_name").length() < 10) {
					tmp = rs.getInt("sales_number") + "\t" + rs.getInt("shoes_serial_number") + "\t"
							+ rs.getString("sh_name") + "\t\t" + rs.getInt("sh_size") + "\t" + rs.getInt("sh_info")
							+ "\t" + rs.getInt("sh_price") + "\n";
				} else {
					tmp = rs.getInt("sales_number") + "\t" + rs.getInt("shoes_serial_number") + "\t"
							+ rs.getString("sh_name") + "\t" + rs.getInt("sh_size") + "\t" + rs.getInt("sh_info") + "\t"
	...
	public void Month_view(JTextArea ta) {
		...
		try {
			ta.setText("  판매번호\t시리얼넘버\t신발이름\t\t사이즈\t판매개수\t가격\n\n");
			conn = ds.getConnection();
			pstmt = conn.prepareStatement("SELECT * FROM product_record " + "WHERE TO_CHAR(sales_time,'YYYYMM') = "
					+ today.substring(0, 6) + " ORDER BY sales_number DESC");
			rs = pstmt.executeQuery();
			while (rs.next()) {
				String tmp = "";
				if (rs.getString("sh_name").length() < 10) {
	...
	public void seach_sales_number(JTextArea ta, int number) {
		...
		try {
			...
			pstmt = conn.prepareStatement(
					"SELECT * FROM product_record " + "WHERE sales_number = " + number + " ORDER BY sales_number DESC");
			rs = pstmt.executeQuery();
			while (rs.next()) {
				String tmp = "";
				if (rs.getString("sh_name").length() < 10) {
	...
	public void seach_member(JTextArea ta, String member) {
		...
		try {
			...
			conn = ds.getConnection();
			pstmt = conn.prepareStatement("SELECT * FROM ABC" + member + " ORDER BY sh_id DESC");
			rs = pstmt.executeQuery();
			while (rs.next()) {
				String tmp = "";
				if (rs.getString("sh_name").length() < 10) {
	...
	public String pay_check(JTextArea ta, String serial, String log) {
		try {
			...
			conn = ds.getConnection();
			pstmt = conn.prepareStatement("SELECT * FROM shoes WHERE shoes_serial_number = ?");
	...
	public int pay_view(JTextArea ta, Map<String, Integer[]> shoes_list) {
		...
		try {
			...
			for (Entry<String, Integer[]> entry : shoes_list.entrySet()) {
				...
				rs.next();
				if (rs.getString("sh_name").length() < 10) {
					tmp = "\t" + rs.getString("sh_name") + "\t\t\t" + entry.getValue()[1] + "\t" + rs.getInt("sh_size")
							+ "\t" + rs.getInt("sh_price") + "\n";
				} else {
					tmp = "\t" + rs.getString("sh_name") + "\t\t" + entry.getValue()[1] + "\t" + rs.getInt("sh_size")
							+ "\t" + rs.getInt("sh_price") + "\n";
